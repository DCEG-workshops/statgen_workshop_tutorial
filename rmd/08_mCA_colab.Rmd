...
<!-- TODO
- Input data paths still reference `/content/drive/MyDrive/statgen_workshop_mosaicism/` — update to real Biowulf data paths:
  - `ref`, `genome_studio_file`, `maps`, `kgp_pfx`, `cnp`, `cyto`, `pre_phasing_data_dir` (lines ~27-34)
  - `circos_conf`, `ideogram_conf`, `ticks_conf`, `mCAs` (lines ~310-313)
- `Rscript /content/drive/MyDrive/statgen_workshop_mosaicism/bin/mocha_plot.12.21.22.R` (lines ~220,229,238,247,256): Update to real path for mocha_plot R script
- udocker is a Colab workaround. On Biowulf, replace `udocker --allow-root` with `docker` or `singularity`
- Google Drive mount instructions in prose (line ~21): Update to reflect Biowulf data setup
-->
...
```{bash}
export analysis_dir="$(pwd)/08_analysis/"
...
export to_bcf="$(pwd)/to_bcf/"
export phasing="$(pwd)/phasing/"
export phasing_by_chrs="$(pwd)/phasing/by_chrs/"
export mocha="$(pwd)/mocha/"
...
```{bash}
udocker --allow-root run -v $(pwd)/ us.gcr.io/mccarroll-mocha/bcftools:1.16-20221221 bash ${analysis_dir}/to_vcf.sh
```
...
#   echo ${chr}
#   udocker --allow-root run -v $(pwd)/ us.gcr.io/mccarroll-mocha/eagle:1.16-20221221 bash ${analysis_dir}/phasing.sh ${chr}
#done
...
```{bash}
udocker --allow-root run -v $(pwd)/ us.gcr.io/mccarroll-mocha/bcftools:1.16-20221221 bash ${analysis_dir}/concat.sh
```
...
```{bash}
udocker --allow-root run -v $(pwd)/ us.gcr.io/mccarroll-mocha/bcftools:1.16-20221221 bash ${analysis_dir}/mocha.sh
```
...
```{r, out.width="600px"}
knitr::include_graphics(paste0(Sys.getenv("plots"), "S6.png"))
```
...
```{r, out.width="600px"}
knitr::include_graphics(paste0(Sys.getenv("plots"), "S9.png"))
```
...
```{r, out.width="600px"}
knitr::include_graphics(paste0(Sys.getenv("plots"), "S8.png"))
```
...
udocker --allow-root run -v $(pwd)alexcoppe/circos:0.69-6  -conf ${circos_conf}
```
...
```{r, out.width="600px"}
knitr::include_graphics(paste0(Sys.getenv("circos"), "mosaic.png"))
```
