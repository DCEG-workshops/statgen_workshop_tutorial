...
<!-- TODO
- ALL path variables in the first bash chunk (lines ~15-32) use hard-coded `/content/` and `/content/drive/MyDrive/statgen_workshop_mosaicism/` paths. Update all of:
  - `analysis_dir`, `temp_dir`, `to_bcf`, `phasing`, `phasing_by_chrs`, `mocha`, `plots`, `circos` — set to real working directory paths
  - `ref`, `genome_studio_file`, `maps`, `kgp_pfx`, `cnp`, `cyto`, `pre_phasing_data_dir` — set to real input data paths
- ALL `udocker run -v /content/` commands: replace `/content/` with real working directory
- `Rscript /content/drive/MyDrive/statgen_workshop_mosaicism/bin/mocha_plot.12.21.22.R` (lines ~209,218,227,236,245): Update to real path for mocha_plot R script
- Second bash chunk with path variables (lines ~300-306): Same path updates as above
- `ls /content/circos` (line ~357): Will be fixed automatically by using `${circos}` variable, but update the variable itself
- udocker is a Colab workaround. If Docker is available, replace `udocker --allow-root` with `docker`
-->
