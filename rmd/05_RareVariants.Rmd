...
<!-- TODO
- Line ~22: `udocker load -i /content/drive/MyDrive/statgen_workshop/containers/staarpipeline.tar` — update path to staarpipeline.tar on Biowulf, or replace with `udocker pull zilinli/staarpipeline:0.9.7`
- udocker is a Colab workaround for Docker. On Biowulf, Docker/Singularity may be available natively — replace `udocker --allow-root` with `docker` or `singularity`
- `pip install udocker` (line ~18): May not be needed on Biowulf if Docker is available
-->
...
```{bash}
ls statgen_workshop_tutorial/src/05_RareVariants/1000G/
```

Create the analysis directory on the VM

```{bash}
mkdir -p $(pwd)/analysis_dir05/
```

We will run the first step, which is to prepare for the 1000 genomes files to be used by the STAAR pipeline.

```{bash}
cat statgen_workshop_tutorial/src/05_RareVariants/1000G/1000G_scripts_part2/Association_Analysis_PreStep_1kG.r
```
...
```{bash}
udocker --allow-root  run -v $(pwd)/ zilinli/staarpipeline:0.9.7 \
        Rscript statgen_workshop_tutorial/src/05_RareVariants/1000G/1000G_scripts_part2/Association_Analysis_PreStep_1kG.r
```

Let's see if the output files are there

```{bash}
ls -l $(pwd)/analysis_dir05/
```

Next step is to simuate the phenotype for analysis

```{bash}
cat statgen_workshop_tutorial/src/05_RareVariants/1000G/1000G_scripts_part2/Example_Simulated_Phenotype.R
```
...
```{bash}
udocker --allow-root  run -v $(pwd)/ zilinli/staarpipeline:0.9.7 \
        Rscript statgen_workshop_tutorial/src/05_RareVariants/1000G/1000G_scripts_part2/Example_Simulated_Phenotype.R
```
...
```{bash}
cat statgen_workshop_tutorial/src/05_RareVariants/1000G/1000G_scripts_part2/Examples_STAARpipeline.R
```
...
```{bash}
udocker --allow-root  run -v $(pwd)/ zilinli/staarpipeline:0.9.7 \
        Rscript statgen_workshop_tutorial/src/05_RareVariants/1000G/1000G_scripts_part2/Examples_STAARpipeline.R
```
