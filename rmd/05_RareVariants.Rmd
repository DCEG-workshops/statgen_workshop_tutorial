...
<!-- TODO
- Line ~22: `udocker load -i /content/drive/MyDrive/statgen_workshop/containers/staarpipeline.tar` — update path to staarpipeline.tar, or replace with `udocker pull zilinli/staarpipeline:0.9.7` if pulling from Docker Hub is acceptable
- Lines ~63,82,106: `udocker run -v /content/` — replace `/content/` with the actual working directory path (e.g., `$(pwd)` or an absolute path)
- udocker is a Colab workaround for Docker. If running on a system with Docker available, replace `udocker --allow-root` with `docker`
- Script paths inside udocker (e.g., `/content/statgen_workshop_tutorial/...`) reference Colab paths mounted into the container — update to match the new volume mount path
-->
...
```{bash}
mkdir -p /content/analysis_dir05/
```
